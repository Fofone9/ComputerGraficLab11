...
void killPlayer()
{
	player.reset();
	system("cls");
	printf("\tChange the world... My final message... Goodbye.");
}
void destroyIfPossible(ivec2 pos, ivec2 displacement, bool Decrease)
{
	...
	if (player)
	{
		ivec2 realPos{ pos.x - 10,pos.y - 10 };
		ivec2 playerPos = player->getPosition();
		if ((ivec2({ realPos.x + displacement.x,realPos.y + displacement.y }) == playerPos && passabilityMap[pos.x][pos.y] == 0)
			|| ivec2({ realPos.x,realPos.y }) == playerPos)
		{
			killPlayer();
			return;
		}
